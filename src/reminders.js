...
function remindersFor(state,user,{canSeeIssue,canAccessBuilding,hasPerm},today){
 ...
 const make=(type,id,due,name,meta)=>{
  ...
  rows.push({id:key,userId:user.id,kind:'reminder',severity:phase,title:`${type==='issue'?'Задача':'Осмотр'}: ${phase==='overdue'?'срок пропущен':phase==='today'?'срок сегодня':'срок завтра'}`,text:name,at:today+'T00:00:00Z',readAt:reads[key]||'',due,...meta});
 };
